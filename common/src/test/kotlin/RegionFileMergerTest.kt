...
class RegionFileMergerTest {
    ...
    @Test
    fun selectedIndicesOnlyCoverChunksInRange() {
        assertEquals(
            setOf(0, 1, 2),
            RegionFileMerger.selectedIndices(0, 0, 0, 2, 0, 0)
        )
        assertEquals(
            setOf(0, 1, 2),
            RegionFileMerger.selectedIndices(1, 0, 32, 34, 0, 0)
        )
    }

    @Test
    fun readChunksCanExtractOnlySelectedIndices() {
        val file = temp.newFile("selected-read.mca").toPath()
        writeMca(file, mapOf((0 to 0) to byteArrayOf(1), (1 to 0) to byteArrayOf(2), (2 to 0) to byteArrayOf(3)))

        val selected = RegionFileMerger.readChunks(file.readBytes(), setOf(0, 2))
        assertEquals(setOf(0, 2), selected.keys)
    }
