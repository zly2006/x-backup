...
- ✨区域回档：可按区域文件（`--mca`）或按区块（`--chunk`）还原，**范围外的玩家不会受到影响**。
...
这个命令会将游戏恢复到编号为1的备份状态。

如果只想还原当前维度的一部分，可以使用`--mca`（按整个区域文件回档）或`--chunk`（按区块回档）。默认情况下，输入的是 MCA 区号或区块坐标；只有加上`--block`时，才会把数字当作方块 X/Z 坐标再换算。

```
/xb restore 1 --mca 0 0 0 0
```

这只会回档当前维度的 `r.0.0.mca`。

```
/xb restore 1 --chunk 0 0 10 10
```

这会回档区块 (0, 0) 到 (10, 10)，范围外的区块保持不变。

```
/xb restore 1 --mca --block 0 0 10 10
```

把数字当作方块坐标，换算成对应的 MCA 区域文件后再整文件回档。

```
/xb restore 1 --chunk --block 0 0 10 10
```

把数字当作方块坐标，换算成区块坐标后再按区块回档。
